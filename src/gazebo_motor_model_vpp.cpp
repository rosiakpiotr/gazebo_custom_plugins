#include "gazebo_custom_plugins/gazebo_motor_model_vpp.hpp"

using namespace gazebo;

GZ_REGISTER_MODEL_PLUGIN(GazeboMotorModelVpp)

/////////////////////////////////////////////////
GazeboMotorModelVpp::GazeboMotorModelVpp()
    : pitchSetpointFilter(PITCH_SETPOINT_FITLER_TIME_CONSTANT),
      yawSetpointFilter(YAW_SETPOINT_FITLER_TIME_CONSTANT),
      // Default PID gains that worked fine.
      pitchPid{0.08, 0.01, 0.08, 1.5, -1.5, 0.588, -0.588},
      rollPid{0.08, 0.01, 0.05, 1.5, -1.5, 0.588, -0.588},
      yawPid{0.08, 0.01, 0.05, 1.5, -1.5, 1.588, -1.588}
{
}

/////////////////////////////////////////////////
void GazeboMotorModelVpp::Load(physics::ModelPtr _model,
                               sdf::ElementPtr _sdf)
{
    this->model = _model;
    this->sdf = _sdf;
    ...
    auto printMissingJoint = [&](std::string joint_name)
    {
        gzerr << "GazeboMotorModelVpp::Load ERROR! Can't get " << joint_name << " joint" << std::endl;
    };

    yawJoint = model->GetJoint("joint_yaw");
    if (!yawJoint)
    {
        printMissingJoint("yaw");
    }

    rollJoint = model->GetJoint("joint_roll");
    if (!rollJoint)
    {
        printMissingJoint("roll");
    }

    pitchJoint = model->GetJoint("joint_pitch");
    if (!pitchJoint)
    {
        printMissingJoint("pitch");
    }
}

void GazeboMotorModelVpp::initROS()
{
    rosNode = gazebo_ros::Node::Get(sdf);

    // jointStatePub =
    //     rosNode->create_publisher<GimbalAngles>(
    //         JOINT_STATE_TOPIC,
    //         10);

    // cmdAnglesSub =
    //     rosNode->create_subscription<GimbalAngles>(
    //         TARGET_ANGLES_TOPIC, 10,
    //         std::bind(&GazeboMotorModelVpp::jointStateCmdCallback,
    //                   this,
    //                   std::placeholders::_1));

    // Create timer thread to periodically publish joint states.
    // jointStatePubTimer =
    //     rosNode->create_wall_timer(
    //         10ms,
    //         std::bind(&GazeboMotorModelVpp::publishAngles, this));
}

/////////////////////////////////////////////////
void GazeboMotorModelVpp::Init()
{
    lastUpdateTime = model->GetWorld()->SimTime();

    // plugin update
    connections.push_back(event::Events::ConnectWorldUpdateBegin(
        boost::bind(&GazeboMotorModelVpp::OnUpdate, this)));
}

/////////////////////////////////////////////////
void GazeboMotorModelVpp::OnUpdate()
{
    common::Time time = model->GetWorld()->SimTime();
    double dt = (time - lastUpdateTime).Double();

    double filteredPitchSetpoint = pitchSetpointFilter.updateFilter(pitchSetpoint, dt);
    double filteredYawSetpoint = yawSetpointFilter.updateFilter(yawSetpoint, dt);

    double pitchError = filteredPitchSetpoint - pitchJoint->Position();
    // double rollError = rollSetpoint - rollJoint->Position();
    double yawError = filteredYawSetpoint - yawJoint->Position();

    // apply forces to move gimbal
    double pitchForce = pitchPid.Update(pitchError, dt);
    pitchJoint->SetForce(0, -pitchForce);

    // double rollForce = rollPid.Update(rollError, dt);
    // rollJoint->SetForce(0, -rollForce);

    double yawForce = yawPid.Update(yawError, dt);
    yawJoint->SetForce(0, -yawForce);

    lastUpdateTime = time;
}

void GazeboMotorModelVpp::setPIDForAxis(std::string axis_name, common::PID controller)
{
    if (axis_name == "joint_roll")
    {
        rollPid = controller;
    }
    else if (axis_name == "joint_yaw")
    {
        yawPid = controller;
    }
    else if (axis_name == "joint_pitch")
    {
        pitchPid = controller;
    }
    else
    {
        // Potentially print some error/warning message here.
    }
}

void GazeboMotorModelVpp::loadPIDsFromSDF()
{
    sdf::ElementPtr control_channels = sdf->GetElement("control_gimbal_channels");
    ...
    while (channel)
    {
        if (channel->HasElement("joint_axis"))
        {
            ...
            if (channel->HasElement("joint_control_pid"))
            {
                ...
                setPIDForAxis(joint_axis, common::PID(p, i, d, iMax, iMin, cmdMax, cmdMin));
            }
